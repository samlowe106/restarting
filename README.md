...
`restart.sh` targets Pop!_OS 24.04 and is portable to any Debian derivative close enough to Ubuntu 24.04 (noble). Everything tied to one specific machine lives in its own directory: `xps-9315/` for the Dell XPS 13 Plus 9315 this was originally written on, `ideapad-82d2/` for the Lenovo IdeaPad Slim 9 14ITL5.
...
`restart.sh` reads `/sys/class/dmi/id/product_name` and runs the matching `<dir>/setup.sh` at the end; on anything else it prints a line and stops there, so the whole run is safe on hardware it was not written for. To run a machine half by hand, or on hardware whose DMI string differs, `FORCE=1 <dir>/setup.sh`.
...
`ideapad-82d2/setup.sh` was written before the first Linux boot on that machine, from the Windows-side hardware inventory, and is a checklist as much as a script. See the comment at the top of that file.
